        ...
        <div class="container-fluid">
			<table class="table table-bordered table-striped">
				...
				<tbody>
					<?php 
					$i = 1;
						$qry = $conn->query("SELECT i.*,s.name as supplier from items i inner join suppliers s on i.supplier_id = s.id order by i.name asc,s.name asc ");
						while($row = $qry->fetch_assoc()):
					?>
						<tr>
							<td class="text-center"><?php echo $i++; ?></td>
							<td nowrap><?php echo date("Y-m-d H:i",strtotime($row['date_created'])) ?></td>
							<td><?php echo $row['name'] ?></td>
							<td>
							<?php if($row['apparatus'] == 1): ?>
                                    Device
                                <?php else: ?>
                                   Component
                                <?php endif; ?>
