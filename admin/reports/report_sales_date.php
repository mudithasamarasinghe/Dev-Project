...
$query = "SELECT date_format(sl.date_created,'%Y-%m-%d') AS nm, si.quantity AS qtysold 
...
if ($start_date && $end_date && isset($_POST['submit'])) {
    $query .= "sl.date_created BETWEEN date_format('".$start_date."','%Y-%m-%d') AND date_format('".$end_date."','%Y-%m-%d') GROUP BY date_format(sl.date_created,'%Y-%m-%d')";
} else{
	$query .= "sl.date_created BETWEEN ".$start_date." AND ".$end_date." GROUP BY sl.date_created";
...
<body>
<div class="row">
<div class="col-md-6"><h3>Date vs. Quantity Sold</h3></div>
