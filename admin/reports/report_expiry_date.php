...
try {
    ...
    $query = "select i.name as name,sl.expiry_date as expiration_date from stock_list sl inner join items i on sl.item_id=i.id where sl.expiry_date between now() and adddate(now(), INTERVAL 30 DAY)
group by i.name;";
