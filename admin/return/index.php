...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title">List of Returns</h3>
        <div class="card-tools">
            <a href="<?php echo base_url ?>admin/index.php?page=return/manage_return"
        ...
        <div class="container-fluid">
            <div class="container-fluid">
                <table class="table table-bordered table-stripped">
                    ...
                    <?php
                    $i = 1;
			  	   $qry = $conn->query("SELECT r.*,(CHAR_LENGTH(stock_ids) - CHAR_LENGTH(REPLACE(stock_ids, ',','')) + 1) sids, s.name as supplier FROM `return_list` r inner join suppliers s on r.supplier_id = s.id group by r.return_code order by r.date_created desc");
                     while ($row = $qry->fetch_assoc()):


                        $qrynm = $conn->query("SELECT CONCAT(firstname,' ',lastname) as fnln FROM users where id = '{$row['return_approval']}'");
                        $k = "Not yet approved";
                        if ($qrynm->num_rows > 0) {
...
<!--                            <td class="text-left">--><?php //echo $kk; ?><!--</td>-->
						...
						<td class="text-left"> <a class="dropdown-item"
                                       href="<?php echo base_url . 'admin/index.php?page=return/view_return&id=' . $row['id'].'&fid=' . $row['form_id'] ?>"
