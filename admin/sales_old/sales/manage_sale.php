                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        if($qry->num_rows >0) {
                            ...
                            $numeric_part = intval(substr($last_sales_code, 3));
                            $next_numeric_part = $numeric_part + 1;

                            // Concatenate the prefix "PO-" with the incremented numeric part to get the next PO code
                            $next_sales_code = "SALE-" . sprintf("%03d", $next_numeric_part);
//                            }
                        } else {
                            $next_sales_code = "SALE-001";
                        }
                        ?>
                ...
                <fieldset>
                    ...
                    <div class="row justify-content-center align-items-end">
                            <?php 
                                $item_arr = array();
                                $unit_arr = array();
                                $item = $conn->query("SELECT i.*,sl.quantity,i.unit FROM `items` i inner join stock_list sl on i.id = sl.item_id  where status = 1 and sl.quantity > 0 group by i.name order by `name` asc");
                                while($row=$item->fetch_assoc()):
                                    $item_arr[$row['id']] = $row;
                                    $unit_arr[$row['id']] = $row['unit'];
                                    $qty_arr[$row['id']]=$row['quantity'];
                                endwhile;
                            ?>
                        <div class="col-md-3">
                            <div class="form-group">
